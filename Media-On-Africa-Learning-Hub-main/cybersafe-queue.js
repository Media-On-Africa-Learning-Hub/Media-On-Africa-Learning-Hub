...
const CyberSafeQueue = (() => {
   ...
   * @param {string} apiBase - Your CyberSafe API base URL
   * @param {Function} onResult - Callback(id, aiResult) when a result comes back
   */
  async function syncPending(apiBase, onResult) {
    ...
    for (const item of pending) {
      try {
        ...
        const response = await fetch(`${apiBase}/api/cybersafe/scan/content`, {
          method: 'POST',
        ...
        await updateItem(item.id, { status: 'pending' }); // reset to retry later
      }
    }
  }
