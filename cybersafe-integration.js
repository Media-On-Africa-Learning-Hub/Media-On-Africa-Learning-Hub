(function () {
  ...
  const showToast = (type, title, message, duration = 6000) => {
    ...
    body.className = "cs-toast-body";

    const strongEl = document.createElement("strong");
    strongEl.textContent = title; // textContent, not innerHTML — title/message may
    const spanEl = document.createElement("span");            // originate from scan results
    spanEl.textContent = message; // (e.g. AI summary text) and must never be parsed as HTML

    body.append(strongEl, spanEl);
  ...
  const initInputDetection = () => {
    // Opt-in only: scanning now requires data-cs-scan="true" on the field itself.
    // Previously this listened on every <input>/<textarea> in the document and
    // reported their contents to the API unless the id happened to be on a
    // 3-item skip list — meaning any new form field (contact name/email/message,
    // login fields, etc.) would silently have its contents sent to CyberSafe as
    // the user typed. Mark fields you actually want scanned, e.g.:
    //   <textarea id="forum-post" data-cs-scan="true">
    const handler = debounce((e) => {
      const target = e.target;
      if (!target || target.dataset?.csScan !== "true") return;
